...
export class SwipeTracker {
  ...
  visitSector(activeTouch: ActiveTouch, sector: HTMLElement): void {
    if (!activeTouch.visited.has(sector)) {
      activeTouch.visited.add(sector);
      this.newSectorCallback(sector);
    }
  }
  ...
  private touchStart(sector: HTMLElement, e: TouchEvent): void {
    for (const touch of e.changedTouches) {
      ...
      this.visitSector(activeTouch, sector);
  ...
  private touchMove(e: TouchEvent): void {
    ...
    for (const touch of e.changedTouches) {
      ...
      if (activeTouch.currentSector !== sectorUnderSwipe) {
        ...
        this.visitSector(activeTouch, sectorUnderSwipe);
